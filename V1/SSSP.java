...
class Parallel{
    ...
    private ConcurrentLinkedQueue<Integer> currBucket;
    ...
    public Parallel(Surface S, Coordinator C, int NT, int NV) {
        ...
        numVertices = NV;
        currBucket = new ConcurrentLinkedQueue<Integer>();
    ...
    public void solve() throws Coordinator.KilledException {
        ...
        for(int i = 0;i<numThreads;i++){
            ...
            DeltaWorker worker = new DeltaWorker(surface, coordinator, startindex, newrange, this,i);
            workers[i] = worker;
        }
    ...
    public void messageQueue(Surface.Request request){
        // find which worker owns the vertex
        // then add it to that worker's queue

        for(DeltaWorker worker : workers){
            if(worker.ownsVertex(request.getVertex())){
                worker.getQueue().add(request);
            }
        }
    }

    public void addNextBucket(int i){
        System.out.println(i + " added to currBucket");
        currBucket.add(i);
    }

    public ConcurrentLinkedQueue<Integer> getCurrBucket(){
        return currBucket;
    }

    public int getNextBucket(){
        int result = Integer.MAX_VALUE;
        for(int next : currBucket){
            if(next < result) result = next;
        }
        System.out.println("get next bucket: " + result);
        return result;
    }
}
...
class DeltaWorker extends Thread {
    ...
    private HashSet<Surface.Vertex> vertices;
    private ConcurrentLinkedQueue<Surface.Request> queue;
    ArrayList<LinkedHashSet<Surface.Vertex>> buckets;    
    ...
    public void run() {
        try{
            c.register();
            s.DeltaSolveParallel(parallel, this);
            c.unregister();
    ...
    // Constructor
    //
    public DeltaWorker(Surface S, Coordinator C, int StartIndex, int Range, Parallel P, int num) {
        s = S;
        c = C;
        parallel = P;
        ...
        queue = new ConcurrentLinkedQueue<Surface.Request>();
        vertices = new HashSet<Surface.Vertex>();
        number = num;

        System.out.println("NEW WORKER: [" + startIndex + ", " + (startIndex + range-1) + "]");
    }

    public ArrayList<LinkedHashSet<Surface.Vertex>> getBuckets(){
        return buckets;
    }

    public void initBuckets(int numBuckets){
        buckets = new ArrayList<LinkedHashSet<Surface.Vertex>>(numBuckets);        
    }
    public boolean ownsVertex(Surface.Vertex v){
        if(vertices.contains(v)) return true;
        return false;
    }

    public void addVertex(Surface.Vertex v){
        vertices.add(v);
    }
    ...
    public ConcurrentLinkedQueue<Surface.Request> getQueue(){
        return queue;
    }
    ...
    public void run() {
        try {
            ...
            if (dijkstra) {
                ...
                System.out.println("supposedly Delta Solve here");
                // s.DeltaSolve();
            }
            c.unregister();
        } catch(Coordinator.KilledException e) { }
...
class Surface {
    ...
    int numBuckets;
    int delta;
    ArrayList<LinkedHashSet<Vertex>> buckets;
    ...
    public void DeltaSolveParallel(Parallel parallel, DeltaWorker worker) throws Coordinator.KilledException {
        numBuckets = 2 * degree;
        delta = maxCoord / degree;

        String workerid = "Worker " + worker.getNum();
        ...
        //add all the vertices to their respective workers
        for(int i = worker.getStartIndex();i<worker.getEndIndex();i++){
            worker.addVertex(vertices[i]);
        }
        
        worker.initBuckets(numBuckets);
        //create the buckets for each thread
        for (int i = 0; i < numBuckets; ++i) {
            worker.getBuckets().add(new LinkedHashSet<Vertex>());
        }

        //if the worker owns the origin vertex, then add it to their bucket
        if(worker.ownsVertex(vertices[0])){
            System.out.println(workerid + " owns origin");
            worker.getBuckets().get(0).add(vertices[0]);
        }
        else{
            System.out.println(workerid + " doesn't own origin");
        }
        ...
        for (;;) {
            if(!started) currBucket ++;
            started =false;
            // STEP ONE: IF ANY OF THE OWNED VERTICES ARE REALISED, RELAX THEM

            LinkedList<Vertex> removed = new LinkedList<Vertex>();
            LinkedList<Request> requests;
            do{
                for(Request req : worker.getQueue()){
                    System.out.println(workerid + " handling message");
                    req.relax(worker.getBuckets());
                }

                while (worker.getBuckets().get(i).size() > 0) {
                    requests = findRequests(worker.getBuckets().get(i), true);  // light relaxations
                    // Move all vertices from bucket i to removed list.
                    removed.addAll(worker.getBuckets().get(i));
                    worker.getBuckets().set(i, new LinkedHashSet<Vertex>());
                    for (Request req : requests) {
                        //if this thread owns the vertex being relaxed, then do it
                        //otherwise send message queue to parallel
                        if(worker.ownsVertex(req.getVertex())) req.relax(worker.getBuckets());
                        else{
                            System.out.println(workerid + " sending message");
                            parallel.messageQueue(req);
                        }
                    }
                }
                parallel.hold();
                if(!worker.getQueue().isEmpty()) System.out.println(workerid + " has requests on the queue");
            } while(!worker.getQueue().isEmpty());
            // Now bucket i is empty. Cyclic barrier awaits here!
            System.out.println(workerid + " reached barrier 1");
            parallel.hold();

            requests = findRequests(removed, false);    // heavy relaxations
            System.out.println(workerid + " found heavy requests");
            for (Request req : requests) {
                req.relax(worker.getBuckets());
            }
            System.out.println(workerid + " completed heavy relaxations");            
            // Find next nonempty bucket.
            int j = i;
            boolean completed = false;
            do {
                j = (j + 1) % numBuckets;
            } while (j != i && worker.getBuckets().get(j).size() == 0);
            if(i == j) completed = true;
            i = j;
            System.out.println(workerid + ":J = " + j);

            if(!completed)parallel.addNextBucket(i);
            parallel.hold();
            i = parallel.getNextBucket();
            if(currBucket > numBuckets) break;
        }

        System.out.println(workerid + " is DONE FAM");
    }
